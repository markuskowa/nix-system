{ ... } :
{
  ...
  testScript = ''
    ...
    # Check for banner
    $client->succeed("sudo -u testUser ssh -i /etc/dummy-ssh-u -o 'StrictHostKeyChecking no' -t server true | grep NixOS");

    # Ensure no banner is plotted when no terminal is allocated
    $client->succeed("sudo -u testUser ssh -i /etc/dummy-ssh-u -o 'StrictHostKeyChecking no' server true | wc -l | grep '^0'");

    # Ensure no banner is plotted for root whatsoever
    $client->succeed("ssh -i /etc/dummy-ssh-r -o 'StrictHostKeyChecking no' -t server true | wc -l | grep '^0'");
  '';
}
