...
in {
  ...
  nodes = {
    ...
    apd = { pkgs, lib, ... }: {
      ...
      services.hostapd-wired.interfaces.eth2 = {
        bridge = "ap";
        settings = {
          ...
          auth_server_addr = "192.168.1.4";
          auth_server_port = 1812;
          auth_server_shared_secret = "radius_password";

          # RADIUS accounting server
          acct_server_addr = "192.168.1.4";
          acct_server_port = 1813;
          acct_server_shared_secret = "radius_password";
        };
      };
      services.hostapd-wired.interfaces.eth3 = {
        bridge = "ap";
        settings = {
          macsec_policy = true;
          # mka_priority = 0;
  ...
  testScript = ''
    ...
    apd.wait_until_succeeds("${pkgs.hostapd}/bin/hostapd_cli -p /run/hostapd-eth2 -i eth2 all_sta | grep AUTHORIZED")
    client.wait_until_succeeds("${pkgs.wpa_supplicant}/bin/wpa_cli -p /run/wpa_supplicant/client status | grep 'EAP state=SUCCESS'")
    client.wait_until_succeeds("ping -c 1 192.168.2.254")

    client.wait_until_succeeds("${pkgs.wpa_supplicant}/bin/wpa_cli -p /run/wpa_supplicant/client logoff")
    # Wait until unauthorized
    apd.wait_until_succeeds("${pkgs.hostapd}/bin/hostapd_cli -p /run/hostapd-eth2 -i eth2 status | grep 'num_sta...=0'")
    client.fail("ping -c 1 192.168.2.254")
  '';
